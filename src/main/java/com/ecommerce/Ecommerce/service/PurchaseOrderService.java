...
/**
 * Service xử lý các logic liên quan đến phiếu nhập hàng (PurchaseOrder).
 * Quản lý việc tạo, xác nhận, hủy, và xuất phiếu nhập hàng dưới dạng PDF.
 */
@Service
public class PurchaseOrderService {
     ...
     * - Hỗ trợ tìm kiếm theo mã phiếu hoặc tên sản phẩm.
     * @param search Từ khóa tìm kiếm (có thể null).
     * @return Danh sách phiếu nhập hàng dưới dạng PurchaseOrderDTO, sắp xếp theo ID giảm dần.
     */
    public List<PurchaseOrderDTO> getAllPurchaseOrders(String search) {
        Sort sort = Sort.by(Sort.Direction.DESC, "id");
        List<PurchaseOrder> purchaseOrders = purchaseOrderRepository.findAll(sort);
        return purchaseOrders.stream()
                .filter(order -> {
                    if (search == null || search.trim().isEmpty()) return true;
                    String searchLower = search.toLowerCase();
    ...
    /**
     * Lấy thông tin chi tiết của một phiếu nhập hàng theo ID.
     * @param id ID của phiếu nhập hàng.
     * @return Optional chứa PurchaseOrderDTO nếu tìm thấy, hoặc empty nếu không tìm thấy.
     */
    public Optional<PurchaseOrderDTO> getPurchaseOrderById(Long id) {
        return purchaseOrderRepository.findById(id)
                .map(this::mapToDTO);
     ...
     * - Kiểm tra tính hợp lệ của dữ liệu đầu vào (mã phiếu, nhà cung cấp, sản phẩm, v.v.).
     * - Tính toán tổng giá trị dựa trên giá nhập và số lượng.
     * @param purchaseOrder Phiếu nhập hàng cần tạo.
     * @return Phiếu nhập hàng đã được lưu.
    ...
    /**
     * Xác nhận một phiếu nhập hàng.
     * - Chỉ cho phép xác nhận phiếu ở trạng thái PENDING.
     * - Cập nhật số lượng tồn kho (VariantSize) dựa trên các mục nhập hàng.
     * @param id ID của phiếu nhập hàng.
     * @return Phiếu nhập hàng đã được xác nhận.
     * @throws IllegalArgumentException Nếu phiếu không tồn tại hoặc không ở trạng thái PENDING.
     */
    @Transactional
    public PurchaseOrder confirmPurchaseOrder(Long id) {
        PurchaseOrder purchaseOrder = purchaseOrderRepository.findById(id)
    ...
    /**
     * Hủy một phiếu nhập hàng.
     * - Chỉ cho phép hủy phiếu ở trạng thái PENDING.
     * @param id ID của phiếu nhập hàng.
     * @return Phiếu nhập hàng đã được hủy.
     * @throws IllegalArgumentException Nếu phiếu không tồn tại hoặc không ở trạng thái PENDING.
     */
    @Transactional
    public PurchaseOrder cancelPurchaseOrder(Long id) {
    ...
    /**
     * Xuất phiếu nhập hàng dưới dạng PDF.
     * - Bao gồm thông tin phiếu, nhà cung cấp, sản phẩm, danh sách mục nhập, và tổng giá trị.
     * @param id ID của phiếu nhập hàng.
     * @return Mảng byte của file PDF.
     * @throws Exception Nếu có lỗi khi tạo PDF (ví dụ: không tìm thấy font).
     * @throws IllegalArgumentException Nếu phiếu nhập hàng không tồn tại.
     */
    public byte[] exportPurchaseOrderToPdf(Long id) throws Exception {
        Optional<PurchaseOrderDTO> purchaseOrderOpt = getPurchaseOrderById(id);
        if (!purchaseOrderOpt.isPresent()) {
    ...
    /**
     * Ánh xạ một phiếu nhập hàng sang PurchaseOrderDTO để trả về cho client.
     * @param order Phiếu nhập hàng cần ánh xạ.
     * @return PurchaseOrderDTO chứa thông tin phiếu nhập hàng.
     */
    private PurchaseOrderDTO mapToDTO(PurchaseOrder order) {
        PurchaseOrderDTO dto = new PurchaseOrderDTO();
        dto.setId(order.getId());
    ...
    /**
     * Kiểm tra tính hợp lệ của dữ liệu đầu vào khi tạo phiếu nhập hàng.
     * @param purchaseOrder Phiếu nhập hàng cần kiểm tra.
    ...
    /**
     * Tính tổng giá trị của phiếu nhập hàng dựa trên giá nhập và số lượng các mục nhập.
     * @param importPrice Giá nhập của sản phẩm.
     * @param items Danh sách các mục nhập hàng.
     * @return Tổng giá trị của phiếu nhập hàng.
     * @throws IllegalArgumentException Nếu kích thước biến thể không tồn tại.
     */
    private double calculateTotalAmount(Double importPrice, List<PurchaseOrderItem> items) {
        double totalAmount = 0;
        for (PurchaseOrderItem item : items) {
