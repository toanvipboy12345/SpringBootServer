...
//             case "topvariantsquantity": // Thêm case mới cho top variants theo quantitySold
//                 return calculateTopVariantsByQuantitySold(startDate, endDate);
//             default:
...
//             Integer quantitySold = ((Number) result[7]).intValue();

//             topVariants.add(new TopVariantRevenueDTO(productId, variantId, name, price, discountPrice, mainImage, revenue, quantitySold));
//         }

//         return topVariants;
//     }

//     private List<TopVariantRevenueDTO> calculateTopVariantsByQuantitySold(LocalDateTime startDate, LocalDateTime endDate) {
//         List<Object[]> results = orderRepository.findTopVariantsByQuantitySold(startDate, endDate);
//         List<TopVariantRevenueDTO> topVariants = new ArrayList<>();
...
public class StatisticsService {
    ...
    private List<TopProductRevenueDTO> calculateTopProductsByRevenue(LocalDateTime startDate, LocalDateTime endDate) {
        ...
        for (Object[] result : results) {
            ...
            String productName = (String) result[1];
            Double revenue = ((Number) result[2]).doubleValue();
            Integer quantitySold = ((Number) result[3]).intValue(); // Lấy quantitySold từ kết quả truy vấn

            topProducts.add(new TopProductRevenueDTO(productId, productName, revenue, quantitySold));
        }
