...
import com.ecommerce.Ecommerce.model.dto.CartDTO;
import com.ecommerce.Ecommerce.service.PaymentService;
import com.ecommerce.Ecommerce.service.PaymentService.PaymentResponseDTO;
import com.ecommerce.Ecommerce.utils.VNPayUtil;
import jakarta.servlet.http.HttpServletRequest;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.http.ResponseEntity;
...
public class PaymentController {
    ...
    @PostMapping("/create")
    public ResponseEntity<Map<String, String>> createPayment(@RequestBody PaymentRequest request) {
        PaymentResponseDTO responseDTO = paymentService.createPayment(
            request.getOrderId(),
            request.getUserId(),
            request.getCartDTO(),
            request.getPaymentMethod(),
        ...
        Payment payment = responseDTO.getPayment();
        Map<String, String> response = new HashMap<>();
        response.put("orderId", payment.getOrderId());
        // Trả về paymentUrl nếu có (chỉ cho VNPAY)
        if (responseDTO.getPaymentUrl() != null) {
            response.put("paymentUrl", responseDTO.getPaymentUrl());
        }
        return ResponseEntity.ok(response);
    }
    ...
    public ResponseEntity<Void> handlePaymentCallback(
        ...
        try {
            ...
            paymentService.updatePaymentStatus(vnp_TxnRef, vnp_TransactionNo, status, vnp_ResponseCode, secureHash);
            System.out.println("Payment status updated to: " + status + " for orderId: " + vnp_TxnRef);
